...
class HomeViewModelTest : KoinTest {
    ...
    @Test
    fun `emit loaded state`() = runTest {
        trainingsRepository.setTrainingBlocks(sampleTrainingBlocks)
        trainingsRepository.setActiveTrainingBlock(sampleActiveTrainingBlock.id)

        viewModel.uiState.test {
            awaitItem() shouldBe HomeUiState()

            awaitItem() shouldBe HomeUiState(sampleActiveTrainingBlock)
        }
    }
}
